...
namespace ChipManager
{
    ...
    public class GameFirebaseActivity : Activity
    {
        ...
        protected override async void OnCreate(Bundle savedInstanceState)
        {
            ...
            game = await FirebaseHelper.Get(StartGameFirebaseActivity.code);
            this.lp = game.getGList();
            //this.lp = StartGameFirebaseActivity.lst;
            /*p = this.lp;
            turnp = p[0];
            ...
            adapter = new PlayerAdapter(this, lp);
            lv.Adapter = adapter;
            small.Text = p[game.turn].getName();
            big.Text = p[game.turn + 1].getName();
            t.Text = p[game.turn].getName();
            

            IDisposable disposable = FirebaseHelper.firebase.Child(FirebaseHelper.database).AsObservable<Game>().Subscribe(d =>
            {
                small.Text = p[game.turn].getName();                
                t.Text = p[game.turn].getName();
                if (game.turn <= p.Count)
                {
